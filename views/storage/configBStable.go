...
func Storage_operateEventsRestore(this js.Value, args []js.Value) interface{} {
	...
	Jq(fmt.Sprintf("button#restore%d", storage.StorageID.Int64)).On("click", js.FuncOf(func(this js.Value, args []js.Value) interface{} {

		url := fmt.Sprintf("%sstorages/%d/r", ApplicationProxyPath, storage.StorageID.Int64)
		method := "put"
	...
	Jq(fmt.Sprintf("button#restore%d", storage.StorageID.Int64)).SetHtml("")
	Jq(fmt.Sprintf("button#restore%d", storage.StorageID.Int64)).Append(buttonTitle.OuterHTML())
...
func Storage_operateEventsClone(this js.Value, args []js.Value) interface{} {
	...
	utils.LoadContent("storage", href, Storage_createCallback, storage)
...
func Storage_operateEventsBorrow(this js.Value, args []js.Value) interface{} {
	...
	Jq("input#bstorage_id").SetVal(storage.StorageID.Int64)
...
func Storage_operateEventsArchive(this js.Value, args []js.Value) interface{} {
	...
	Jq(fmt.Sprintf("button#archive%d", storage.StorageID.Int64)).On("click", js.FuncOf(func(this js.Value, args []js.Value) interface{} {

		url := fmt.Sprintf("%sstorages/%d/a", ApplicationProxyPath, storage.StorageID.Int64)
		method := "delete"
	...
	Jq(fmt.Sprintf("button#archive%d", storage.StorageID.Int64)).SetHtml("")
	Jq(fmt.Sprintf("button#archive%d", storage.StorageID.Int64)).Append(buttonTitle.OuterHTML())
...
func Storage_operateEventsDelete(this js.Value, args []js.Value) interface{} {
	...
	Jq(fmt.Sprintf("button#delete%d", storage.StorageID.Int64)).On("click", js.FuncOf(func(this js.Value, args []js.Value) interface{} {

		url := fmt.Sprintf("%sstorages/%d", ApplicationProxyPath, storage.StorageID.Int64)
		method := "delete"
	...
	Jq(fmt.Sprintf("button#delete%d", storage.StorageID.Int64)).SetHtml("")
	Jq(fmt.Sprintf("button#delete%d", storage.StorageID.Int64)).Append(buttonTitle.OuterHTML())
...
func Storage_productFormatter(this js.Value, args []js.Value) interface{} {
	...
	for _, chunk := range utils.Chunks(storage.Product.Name.NameLabel, 40) {
		d.AppendChild(
...
func Storage_storelocationFormatter(this js.Value, args []js.Value) interface{} {
	...
	iconColor := widgets.NewIcon(widgets.IconAttributes{
		BaseAttributes: widgets.BaseAttributes{
			...
			Attributes: map[string]string{
				"style": fmt.Sprintf("color: %s", storage.StoreLocation.StoreLocationColor.String),
			},
		},
	...
	spanLabel := widgets.NewSpan(widgets.SpanAttributes{
		...
		Text: storage.StoreLocation.StoreLocationFullPath,
	})
...
func Storage_quantityFormatter(this js.Value, args []js.Value) interface{} {
	...
	return fmt.Sprintf("%v %s", storage.StorageQuantity.Float64, storage.UnitQuantity.UnitLabel.String)
...
func Storage_barecodeFormatter(this js.Value, args []js.Value) interface{} {
	...
	for _, chunk := range utils.Chunks(storage.StorageBarecode.String, 10) {
		d.AppendChild(
			widgets.NewSpan(widgets.SpanAttributes{
				BaseAttributes: widgets.BaseAttributes{
					Visible: true,
...
func Storage_operateFormatter(this js.Value, args []js.Value) interface{} {
	...
	if storage.Borrowing == nil || !storage.Borrowing.BorrowingID.Valid {
	...
	if storage.StorageArchive.Valid && storage.StorageArchive.Bool {

		// This is an archive.
		buttonClone = widgets.NewBSButtonWithIcon(
			widgets.ButtonAttributes{
				BaseAttributes: widgets.BaseAttributes{
					Id:      "clone" + strconv.Itoa(int(storage.StorageID.Int64)),
		...
		buttonRestore = widgets.NewBSButtonWithIcon(
			widgets.ButtonAttributes{
				BaseAttributes: widgets.BaseAttributes{
					Id:         "restore" + strconv.Itoa(int(storage.StorageID.Int64)),
					Classes:    []string{"restore"},
					Visible:    false,
					Attributes: map[string]string{"sid": strconv.Itoa(int(storage.StorageID.Int64))},
				},
		...
		buttonDelete = widgets.NewBSButtonWithIcon(
			widgets.ButtonAttributes{
				BaseAttributes: widgets.BaseAttributes{
					Id:      "delete" + strconv.Itoa(int(storage.StorageID.Int64)),
	...
	} else if storage.Storage.StorageID.Valid {
		...
		buttonEdit = widgets.NewBSButtonWithIcon(
			widgets.ButtonAttributes{
				BaseAttributes: widgets.BaseAttributes{
					Id:      "edit" + strconv.Itoa(int(storage.StorageID.Int64)),
		...
		buttonArchive = widgets.NewBSButtonWithIcon(
			widgets.ButtonAttributes{
				BaseAttributes: widgets.BaseAttributes{
					Id:      "archive" + strconv.Itoa(int(storage.StorageID.Int64)),
		...
		buttonBorrow = widgets.NewBSButtonWithIcon(
			widgets.ButtonAttributes{
				BaseAttributes: widgets.BaseAttributes{
					Id:         "borrow" + strconv.Itoa(int(storage.StorageID.Int64)),
	...
	if storage.StorageHC != 0 {

		buttonHistory = widgets.NewBSButtonWithIcon(
			widgets.ButtonAttributes{
				BaseAttributes: widgets.BaseAttributes{
					Id:      "history" + strconv.Itoa(int(storage.StorageID.Int64)),
	...
	spanId := widgets.NewSpan(widgets.SpanAttributes{
		...
		Text: strconv.Itoa(int(storage.StorageID.Int64)),
	}).OuterHTML()
...
func DetailFormatter(this js.Value, args []js.Value) interface{} {
	...
	if len(storage.StorageQRCode) == 0 {
		...
		qrCode = base64.StdEncoding.EncodeToString(storage.StorageQRCode)
	}
	...
	if storage.StorageToDestroy.Valid && storage.StorageToDestroy.Bool {
	...
	colProduct.AppendChild(
		widgets.NewSpan(widgets.SpanAttributes{
			...
			Text: storage.Product.Name.NameLabel,
		}))
	// Store location.
	...
	colQuantity.AppendChild(
		widgets.NewSpan(widgets.SpanAttributes{
			...
			Text: fmt.Sprintf("%v %s", storage.StorageQuantity.Float64, storage.UnitQuantity.UnitLabel.String),
		}))
	// Barecode.
	...
	colBarecode.AppendChild(
		widgets.NewSpan(widgets.SpanAttributes{
			...
			Text: storage.StorageBarecode.String,
		}))
	...
	if storage.StorageConcentration.Valid {
		...
		colConcentration.AppendChild(
			widgets.NewSpan(widgets.SpanAttributes{
				...
				Text: fmt.Sprintf("%d %s", storage.StorageConcentration.Int64, storage.UnitConcentration.UnitLabel.String),
			}))
	}
	// Batch number.
	...
	if storage.StorageBatchNumber.Valid && storage.StorageBatchNumber.String != "" {
		...
		colBatchnumber.AppendChild(
			widgets.NewSpan(widgets.SpanAttributes{
				...
				Text: storage.StorageBatchNumber.String,
			}))
	}
	...
	if storage.Supplier.SupplierID.Valid {
		...
		colSupplier.AppendChild(
			widgets.NewSpan(widgets.SpanAttributes{
				...
				Text: storage.Supplier.SupplierLabel.String,
			}))
	}
	// Reference.
	...
	if storage.StorageReference.Valid {
		...
		colReference.AppendChild(
			widgets.NewSpan(widgets.SpanAttributes{
				...
				Text: storage.StorageReference.String,
			}))
	}
	...
	if storage.StorageEntryDate.Valid {
		...
		colEntryDate.AppendChild(
			widgets.NewSpan(widgets.SpanAttributes{
				...
				Text: storage.StorageEntryDate.Time.String(),
			}))
	}
	// Exit date.
	...
	if storage.StorageExitDate.Valid {
		...
		colExitDate.AppendChild(
			widgets.NewSpan(widgets.SpanAttributes{
				...
				Text: storage.StorageExitDate.Time.String(),
			}))
	}
	// Opening date.
	...
	if storage.StorageOpeningDate.Valid {
		...
		colOpeningDate.AppendChild(
			widgets.NewSpan(widgets.SpanAttributes{
				...
				Text: storage.StorageOpeningDate.Time.String(),
			}))
	}
	// Expiration date.
	...
	if storage.StorageExpirationDate.Valid {
		...
		colExpirationDate.AppendChild(
			widgets.NewSpan(widgets.SpanAttributes{
				...
				Text: storage.StorageExpirationDate.Time.String(),
			}))
	}
	...
	if storage.StorageComment.Valid && storage.StorageComment.String != "" {
		...
		colComment.AppendChild(
			widgets.NewSpan(widgets.SpanAttributes{
				...
				Text: storage.StorageComment.String,
			}))
	}
	...
	colCreationDate.AppendChild(
		widgets.NewSpan(widgets.SpanAttributes{
			...
			Text: storage.StorageCreationDate.String(),
		}))
	// Person.
	...
	colPerson.AppendChild(widgets.NewSpan(widgets.SpanAttributes{
		...
		Text: storage.Person.PersonEmail,
	}))
